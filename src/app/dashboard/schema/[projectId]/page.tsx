...
import { useRouter } from "next/router"; // assuming you're using the pages directory
import { useDispatch, useSelector } from "react-redux";
import {
  setSchema,
  setRLSPolicies,
...
export default function SchemaPage() {
  ...
  const { projectId } = router.query; // projectId should be present in the URL
  const [saving, setSaving] = useState(false);

  // Other state declarations from your original component:
  const [nodes, setNodes, onNodesChange] = useNodesState([]);
  const [edges, setEdges, onEdgesChange] = useEdgesState([]);
  const [expandedTests, setExpandedTests] = useState<string[]>([]);
  const [testCategories, setTestCategories] = useState<TestCategory[]>([]);
  ...
  const [providerConfigs, setProviderConfigs] = useState<
    Record<string, { url: string; key: string }>
  >({
    supabase: { url: supabaseConfig.url, key: supabaseConfig.anonKey },
    firebase: { url: "", key: "" },
    aws: { url: "", key: "" },
  ...
  // Auto-save effect: when schema, RLS policies, or additional context change, save after 1s debounce
  useEffect(() => {
  ...
  }, [schema, rlsPolicies, additionalContext, projectId]);
    ...
    <div className="h-screen flex">
      ...
      <div className="w-1/2 p-4 overflow-y-auto border-r">
              ...
              <PopoverTrigger asChild>
                <Button
                  ...
                  className={`h-9 px-4 py-2 rounded-full flex items-center gap-2 transition-colors ${
                    activeProvider === provider.id ? "bg-primary text-primary-foreground" : "hover:bg-muted"
                  }`}
                  onClick={() => setActiveProvider(provider.id)}
                >
                  <Image
        ...
        <div className="space-y-4">
          ...
          {saving && <p className="text-sm text-blue-500">Auto-saving...</p>}
        </div>
      </div>
